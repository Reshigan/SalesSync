...
api.delete('/regions/:id', async (c) => {
  ...
  try {
    ...
    await db.prepare('DELETE FROM regions WHERE id = ? AND tenant_id = ?').bind(id, tenantId).run();
...
api.delete('/org-hierarchy/:id', async (c) => {
  const db = c.env.DB; const tenantId = c.get('tenantId'); const { id } = c.req.param();
  try {
    await db.prepare('DELETE FROM org_hierarchy WHERE id = ? AND tenant_id = ?').bind(id, tenantId).run();
...
api.delete('/agent-targets/:id', async (c) => {
  ...
  try {
    ...
    await db.prepare('DELETE FROM agent_targets WHERE id = ? AND tenant_id = ?').bind(id, tenantId).run();
...
// ===== Added route aliases and missing endpoints for test coverage (mounted before app.route) =====
api.get('/dashboard/sales-trend', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const { results } = await db.prepare("SELECT strftime('%Y-%m', created_at) as month, SUM(total_amount) as revenue, COUNT(*) as orders FROM orders WHERE tenant_id = ? GROUP BY month ORDER BY month DESC LIMIT 12").bind(tenantId).all(); return c.json({ success: true, data: results || [] }); } catch (e) { return c.json({ success: true, data: [] }); } });
api.get('/analytics/overview', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const [{ results: orders = [] } , { results: customers = [] }, { results: products = [] }] = await Promise.all([ db.prepare('SELECT COUNT(*) as count FROM orders WHERE tenant_id = ?').bind(tenantId).all(), db.prepare('SELECT COUNT(*) as count FROM customers WHERE tenant_id = ?').bind(tenantId).all(), db.prepare('SELECT COUNT(*) as count FROM products WHERE tenant_id = ?').bind(tenantId).all() ]); return c.json({ success: true, data: { orders: orders?.[0]?.count || 0, customers: customers?.[0]?.count || 0, products: products?.[0]?.count || 0 } }); } catch { return c.json({ success: true, data: { orders: 0, customers: 0, products: 0 } }); } });
api.get('/analytics/field-operations', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const { results } = await db.prepare('SELECT COUNT(*) as total_visits FROM visits WHERE tenant_id = ?').bind(tenantId).all(); return c.json({ success: true, data: { total_visits: results?.[0]?.total_visits || 0 } }); } catch { return c.json({ success: true, data: { total_visits: 0 } }); } });
api.get('/analytics/commissions', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const { results } = await db.prepare('SELECT status, SUM(amount) as total_amount, COUNT(*) as count FROM commissions WHERE tenant_id = ? GROUP BY status').bind(tenantId).all(); return c.json({ success: true, data: results || [] }); } catch { return c.json({ success: true, data: [] }); } });
api.get('/orders/dashboard', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const { results } = await db.prepare('SELECT order_status as status, COUNT(*) as count FROM orders WHERE tenant_id = ? GROUP BY order_status').bind(tenantId).all(); return c.json({ success: true, data: results || [] }); } catch { return c.json({ success: true, data: [] }); } });
...
api.post('/inventory/adjustments', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); const userId = c.get('userId'); try { const body = await c.req.json(); const id = (globalThis.crypto?.randomUUID?.() || undefined) ? crypto.randomUUID() : (Date.now().toString(36)); const adjNo = `ADJ-${Date.now().toString(36).toUpperCase()}`; await db.prepare('INSERT INTO inventory_adjustments (id, tenant_id, adjustment_number, warehouse_id, adjustment_date, reason, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(id, tenantId, adjNo, body.warehouse_id || null, new Date().toISOString(), body.reason || null, 'draft', userId || null).run(); return c.json({ success: true, data: { id } }, 201); } catch (e) { return c.json({ success: false, error: e.message }, 500); } });
api.get('/finance/commission-payouts', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const { results } = await db.prepare('SELECT * FROM commission_payouts WHERE tenant_id = ? ORDER BY created_at DESC').bind(tenantId).all(); return c.json({ success: true, data: results || [] }); } catch { return c.json({ success: true, data: [] }); } });
api.get('/van-sales/pipeline', async (c) => { const db = c.env.DB; const tenantId = c.get('tenantId'); try { const { results } = await db.prepare('SELECT status, COUNT(*) as count FROM van_loads WHERE tenant_id = ? GROUP BY status').bind(tenantId).all(); return c.json({ success: true, data: results || [] }); } catch { return c.json({ success: true, data: [] }); } });
