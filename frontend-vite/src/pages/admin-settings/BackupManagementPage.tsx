...
export const BackupManagementPage: React.FC = () => {
  ...
  const backups: Backup[] = Array.isArray(backupsData) ? backupsData : []

  const createBackupMutation = useMutation({
    mutationFn: async (type: string) => {
      const res = await apiClient.post('/system/backups', { type, name: `${type}_backup_${new Date().toISOString().slice(0,10)}` })
      return res.data
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Backup Management</h1>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => toast.success('Schedule updated')} className="text-blue-600 hover:text-blue-900 text-sm font-medium">
                Edit
              </button>
            </div>
          </div>
                ...
                {backups.map((backup) => (
                  <tr key={backup.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-3 sm:px-4 whitespace-nowrap text-sm font-medium">
                      <button onClick={() => toast.success('Restore initiated')} className="text-blue-600 hover:text-blue-900 mr-4">
                        Restore
                      </button>
                      <button onClick={() => toast.success('Download started')} className="text-green-600 hover:text-green-900 mr-4">
                        Download
                      </button>
                      <button onClick={() => { if (confirm('Delete this backup?')) toast.success('Backup deleted') }} className="text-red-600 hover:text-red-900">
                        Delete
                      </button>
                    </td>
                  </tr>
