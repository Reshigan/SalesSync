...
export const ReturnManagementPage: React.FC = () => {
  ...
  const returns: Return[] = (returnsData?.data || returnsData?.returns || []).map((r: any) => ({
    id: r.id,
    order_id: r.order_id,
    order_number: r.order_number || r.order?.order_number || 'N/A',
    customer_name: r.customer_name || r.customer?.name || 'Unknown Customer',
    return_date: r.return_date || r.created_at,
    reason: r.reason || '',
    status: r.status || 'pending',
    refund_amount: r.refund_amount || r.amount || 0,
    items_count: r.items_count || r.items?.length || 0
  }))
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {filteredReturns.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">No return requests found for the selected filter.</p>
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredReturns.map((returnItem) => (
                  <tr key={returnItem.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{returnItem.order_number}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{returnItem.items_count}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadge(returnItem.status)}`}>
                        {returnItem.status}
                      </span>
                    </td>
