...
export default function CommissionRuleDetail() {
  ...
  if (!rule) return <div className="p-6"><div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-yellow-700">Commission Rule not found</div></div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/commissions/rules')} className="p-2 hover:bg-gray-100 rounded-lg"><ArrowLeft className="h-5 w-5" /></button>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{rule.name || rule.title || `Commission Rule #${id}`}</h1>
            <span className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${sc[rule.status] || 'bg-gray-100 text-gray-800'}`}>{(rule.status || 'N/A').replace(/_/g, ' ')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div><dt className="text-sm text-gray-500">Type</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{rule.rule_type || rule.type || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Rate</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{rule.rate ? `${rule.rate}%` : null || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Fixed Amount</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{rule.fixed_amount ? formatCurrency(rule.fixed_amount) : null || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Product</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{rule.product_name || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Category</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{rule.category || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Effective Date</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{formatDate(rule.effective_date) || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Expiry Date</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{formatDate(rule.expiry_date) || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Created At</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{formatDate(rule.created_at) || '-'}</dd></div>
        </dl>
      </div>
