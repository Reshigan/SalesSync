...
export const CompliancePage: React.FC = () => {
  ...
  useEffect(() => {
    apiClient.get('/kyc/compliance').then(res => {
      const data = res.data?.data || res.data
      if (Array.isArray(data)) setComplianceItems(data)
    }).catch(() => {})
  }, [])
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Compliance Management</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {filteredCompliance.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">Compliance requirements will appear here.</p>
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredCompliance.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-3 sm:px-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getRiskBadge(item.risk_level)}`}>
                        {item.risk_level}
                      </span>
                    </td>
                    <td className="px-3 py-3 sm:px-4 whitespace-nowrap">
