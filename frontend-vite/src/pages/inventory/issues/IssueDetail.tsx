...
export default function IssueDetail() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_CONFIG.BASE_URL}/inventory/issues/${id}`, { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-lg shadow p-4"><div className="flex items-center gap-3"><Package className="h-8 w-8 text-blue-500" /><div><p className="text-sm text-gray-500">Items</p><p className="text-xl font-bold">{(issue.items||[]).length}</p></div></div></div>
        <div className="bg-white rounded-lg shadow p-4"><div className="flex items-center gap-3"><Calendar className="h-8 w-8 text-purple-500" /><div><p className="text-sm text-gray-500">Date</p><p className="text-xl font-bold">{formatDate(issue.issue_date || issue.date) || 'N/A'}</p></div></div></div>
        <div className="bg-white rounded-lg shadow p-4"><div className="flex items-center gap-3"><User className="h-8 w-8 text-green-500" /><div><p className="text-sm text-gray-500">Requested By</p><p className="text-xl font-bold">{issue.requested_by || 'N/A'}</p></div></div></div>
      </div>
      ...
      {issue.notes && <div className="bg-white rounded-lg shadow p-6"><h2 className="text-lg font-semibold mb-2">Notes</h2><p className="text-gray-700 whitespace-pre-wrap">{issue.notes}</p></div>}
    </div>
  )
}
