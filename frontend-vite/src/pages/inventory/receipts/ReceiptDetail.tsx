...
export default function ReceiptDetail() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_CONFIG.BASE_URL}/inventory/receipts/${id}`, { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  if (!receipt) return <div className="p-6"><div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-yellow-700">Receipt not found</div></div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/inventory/receipts')} className="p-2 hover:bg-gray-100 rounded-lg"><ArrowLeft className="h-5 w-5" /></button>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{receipt.name || receipt.title || `Receipt #${id}`}</h1>
            <span className={`inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${sc[receipt.status] || 'bg-gray-100 text-gray-800'}`}>{(receipt.status || 'N/A').replace(/_/g, ' ')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div><dt className="text-sm text-gray-500">Status</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{receipt.status || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Reference</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{receipt.reference || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Warehouse</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{receipt.warehouse_name || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Created By</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{receipt.created_by || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Created At</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{formatDate(receipt.created_at) || '-'}</dd></div>
          <div><dt className="text-sm text-gray-500">Updated At</dt><dd className="text-sm font-medium text-gray-900 mt-0.5">{formatDate(receipt.updated_at) || '-'}</dd></div>
        </dl>
      </div>
