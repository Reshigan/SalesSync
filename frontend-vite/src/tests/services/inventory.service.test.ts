...
describe('Inventory Service Tests', () => {
  ...
  describe('getInventory', () => {
    ...
    it('should fetch inventory with warehouse filter', async () => {
      ...
      const { inventoryService } = await import('../../services/inventory.service')
      await inventoryService.getInventory({ warehouse_id: 'w1' })
      expect(apiClient.get).toHaveBeenCalled()
    })
    it('should fetch inventory with product filter', async () => {
      (apiClient.get as any).mockResolvedValue({ data: { data: [], total: 0 } })
      const { inventoryService } = await import('../../services/inventory.service')
      await inventoryService.getInventory({ product_id: 'p1' })
      expect(apiClient.get).toHaveBeenCalled()
    })
    it('should fetch low stock items', async () => {
      (apiClient.get as any).mockResolvedValue({ data: { data: [], total: 0 } })
      const { inventoryService } = await import('../../services/inventory.service')
      await inventoryService.getInventory({ low_stock: true })
      expect(apiClient.get).toHaveBeenCalled()
    })
    it('should handle pagination', async () => {
      (apiClient.get as any).mockResolvedValue({ data: { data: [], total: 0 } })
      const { inventoryService } = await import('../../services/inventory.service')
      await inventoryService.getInventory({ page: 1, limit: 25 })
      expect(apiClient.get).toHaveBeenCalled()
    })
    it('should handle search', async () => {
      (apiClient.get as any).mockResolvedValue({ data: { data: [], total: 0 } })
      const { inventoryService } = await import('../../services/inventory.service')
      await inventoryService.getInventory({ search: 'widget' })
      expect(apiClient.get).toHaveBeenCalled()
    })
    ...
    it('should handle server error', async () => {
      ...
      const { inventoryService } = await import('../../services/inventory.service')
      await expect(inventoryService.getInventory()).rejects.toBeDefined()
    })
  })
