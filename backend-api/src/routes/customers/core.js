...
router.post('/', requireFunction('customers', 'create'), asyncHandler(async (req, res, next) => {
  ...
  return sendCreated(res, { customer: createdCustomer }, 'Customer created successfully');
}));
...
router.get('/:id', requireFunction('customers', 'view'), asyncHandler(async (req, res, next) => {
  ...
  const recentVisits = await getQuery(`
    SELECT v.id, v.visit_date, v.visit_type, v.status, u.first_name, u.last_name
    FROM visits v
  ...
  return sendSuccess(res, { customer, recentOrders, recentVisits });
}));
...
router.put('/:id', requireFunction('customers', 'edit'), asyncHandler(async (req, res, next) => {
  ...
  return sendSuccess(res, { customer: updatedCustomer }, 'Customer updated successfully');
}));
...
router.delete('/:id', requireFunction('customers', 'delete'), asyncHandler(async (req, res, next) => {
  ...
  if (hasOrders && hasOrders.count > 0) {
    ...
    await runQuery(
      'UPDATE customers SET status = ? WHERE id = ? AND tenant_id = ?',
      ['inactive', id, req.tenantId]
    );
    return sendSuccess(res, null, 'Customer deactivated (has related orders)');
  }
