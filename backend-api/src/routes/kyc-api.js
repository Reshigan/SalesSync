...
router.get('/:id', async (req, res) => {
  try {
    ...
    db.get(`SELECT k.*, c.name as customer_name, c.email, c.phone, c.address,
      u.first_name || ' ' || u.last_name as verified_by_name FROM kyc_submissions k
...
router.post('/', async (req, res) => {
  try {
    ...
    db.run(`INSERT INTO kyc_submissions (tenant_id, reference_number, customer_id, document_type, document_number, document_url, expiry_date, notes, status, uploaded_by, created_at)
...
router.post('/:id/verify', async (req, res) => {
  try {
    ...
    db.run(`UPDATE kyc_submissions SET status = 'verified', verification_notes = ?, verified_by = ?, verified_at = datetime('now') WHERE id = ? AND tenant_id = ? AND status = 'pending'`,
...
router.post('/:id/reject', async (req, res) => {
  try {
    ...
    db.run(`UPDATE kyc_submissions SET status = 'rejected', rejection_reason = ?, verified_by = ?, verified_at = datetime('now') WHERE id = ? AND tenant_id = ? AND status = 'pending'`,
