...
router.get('/', async (req, res) => {
  try {
    ...
    let sql = `SELECT k.*, c.name as customer_name, c.email, c.phone FROM kyc_documents k
      LEFT JOIN customers c ON k.customer_id = c.id WHERE k.tenant_id = $1`;
...
router.post('/:id/verify', async (req, res) => {
  try {
    ...
    db.run(`UPDATE kyc_documents SET status = 'verified', verification_notes = $1, verified_by = $2, verified_at = CURRENT_TIMESTAMP WHERE id = $3 AND tenant_id = $4 AND status = 'pending'`,
...
router.post('/:id/reject', async (req, res) => {
  try {
    ...
    db.run(`UPDATE kyc_documents SET status = 'rejected', rejection_reason = $1, verified_by = $2, verified_at = CURRENT_TIMESTAMP WHERE id = $3 AND tenant_id = $4 AND status = 'pending'`,
...
router.get('/stats/summary', async (req, res) => {
  try {
    ...
    db.get(`SELECT 
      ...
      FROM kyc_documents WHERE tenant_id = $1`,
      [tenantId],
      (err, summary) => {
