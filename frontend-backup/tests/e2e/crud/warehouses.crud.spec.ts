...
test.describe('Warehouses CRUD E2E Tests', () => {
  let helper: TestHelper;

  test.beforeEach(async ({ page }) => {
    helper = new TestHelper(page);
    await helper.login();
    await helper.goto('/inventory/warehouses');
  });
  ...
  test('should open create warehouses form', async ({ page }) => {
  ...
  test('should create new warehouses', async ({ page }) => {
    const createButton = page.locator('button:has-text("Add"), button:has-text("Create"), button:has-text("New"), a:has-text("Add"), a:has-text("Create"), a:has-text("New")');
    ...
    if (await createButton.first().isVisible().catch(() => false)) {
      ...
      const codeInput = page.locator('input[name="code"], textarea[name="code"]');
      if (await codeInput.isVisible().catch(() => false)) {
        await codeInput.fill('Test code ' + Date.now());
      }

      const addressInput = page.locator('input[name="address"], textarea[name="address"]');
      if (await addressInput.isVisible().catch(() => false)) {
        await addressInput.fill('Test address ' + Date.now());
      }
  ...
  test('should view warehouses details', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should edit warehouses', async ({ page }) => {
    await page.waitForLoadState('networkidle');
