...
test.describe('Orders CRUD E2E Tests', () => {
  let helper: TestHelper;

  test.beforeEach(async ({ page }) => {
    helper = new TestHelper(page);
    await helper.login();
    await helper.goto('/orders');
  });
  ...
  test('should open create orders form', async ({ page }) => {
  ...
  test('should create new orders', async ({ page }) => {
    const createButton = page.locator('button:has-text("Add"), button:has-text("Create"), button:has-text("New"), a:has-text("Add"), a:has-text("Create"), a:has-text("New")');
    ...
    if (await createButton.first().isVisible().catch(() => false)) {
      ...
      const customerInput = page.locator('input[name="customer"], textarea[name="customer"]');
      if (await customerInput.isVisible().catch(() => false)) {
        await customerInput.fill('Test customer ' + Date.now());
      }

      const statusInput = page.locator('input[name="status"], textarea[name="status"]');
      if (await statusInput.isVisible().catch(() => false)) {
        await statusInput.fill('Test status ' + Date.now());
      }
  ...
  test('should view orders details', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should edit orders', async ({ page }) => {
    await page.waitForLoadState('networkidle');
    ...
    if (await editButton.isVisible().catch(() => false)) {
      ...
      const firstInput = page.locator('input[name="customer"], textarea[name="customer"]');
