...
test.describe('Agents CRUD E2E Tests', () => {
  let helper: TestHelper;

  test.beforeEach(async ({ page }) => {
    helper = new TestHelper(page);
    await helper.login();
    await helper.goto('/agents');
  });
  ...
  test('should open create agents form', async ({ page }) => {
  ...
  test('should create new agents', async ({ page }) => {
    const createButton = page.locator('button:has-text("Add"), button:has-text("Create"), button:has-text("New"), a:has-text("Add"), a:has-text("Create"), a:has-text("New")');
    ...
    if (await createButton.first().isVisible().catch(() => false)) {
      ...
      const emailInput = page.locator('input[name="email"], textarea[name="email"]');
      if (await emailInput.isVisible().catch(() => false)) {
        await emailInput.fill('Test email ' + Date.now());
      }

      const phoneInput = page.locator('input[name="phone"], textarea[name="phone"]');
      if (await phoneInput.isVisible().catch(() => false)) {
        await phoneInput.fill('Test phone ' + Date.now());
      }
  ...
  test('should view agents details', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should edit agents', async ({ page }) => {
    await page.waitForLoadState('networkidle');
