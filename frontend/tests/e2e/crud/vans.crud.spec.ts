...
test.describe('Vans CRUD E2E Tests', () => {
  let helper: TestHelper;

  test.beforeEach(async ({ page }) => {
    helper = new TestHelper(page);
    await helper.login();
    await helper.goto('/fleet/vans');
  });

  test('should list vans', async ({ page }) => {
  ...
  test('should search vans', async ({ page }) => {
  ...
  test('should open create vans form', async ({ page }) => {
  ...
  test('should create new vans', async ({ page }) => {
    const createButton = page.locator('button:has-text("Add"), button:has-text("Create"), button:has-text("New"), a:has-text("Add"), a:has-text("Create"), a:has-text("New")');
    ...
    if (await createButton.first().isVisible().catch(() => false)) {
      ...
      const registrationNumberInput = page.locator('input[name="registrationNumber"], textarea[name="registrationNumber"]');
      if (await registrationNumberInput.isVisible().catch(() => false)) {
        await registrationNumberInput.fill('Test registrationNumber ' + Date.now());
      }

      const modelInput = page.locator('input[name="model"], textarea[name="model"]');
  ...
  test('should view vans details', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should edit vans', async ({ page }) => {
    await page.waitForLoadState('networkidle');
    ...
    if (await editButton.isVisible().catch(() => false)) {
      ...
      const firstInput = page.locator('input[name="registrationNumber"], textarea[name="registrationNumber"]');
